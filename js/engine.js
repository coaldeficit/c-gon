    ...
    for (let i = 0, j = pairs.length; i != j; i++) {
        ...
        for (let k = 0; k < mob.length; k++) {
			...
			if (hasStomped === false) {
                        ...
                        if (tech.isAnnihilation && !mob[k].shield && !mob[k].isShielded && !mob[k].isBoss && mob[k].isDropPowerUp && m.energy > 0.34) {
                            m.energy -= 0.33 //0.33 * m.energy
                        ...
                        if (obj.classType === "bullet" && obj.speed > obj.minDmgSpeed) {
                            ...
                            // console.log(dmg)
                            mob[k].damage(dmg);
                            if (mob[k].alive && !mob[k].painless) mob[k].foundPlayer();
