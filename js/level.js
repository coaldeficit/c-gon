    ...
    nextLevel() {
        if (!level.disableExit) {
            ...
            if (simulation.isTraining) {
				...
				if (simulation.specialMode) simulation.difficultyMode += 1.5
            }
    ...
    populateLevels() { //run a second time if URL is loaded
        ...
        if (simulation.isTraining) {
            ...
            if (simulation.isCommunityMaps) {
                ...
                level.levels.splice(0, (level.communityLevels.length-2)+level.jgonLevels.length); //remove some random levels to make up for adding the community levels
                simulation.isHorizontalFlipped = false;
            } else {
                level.levels = level.levels.concat(level.cgonLevels)
                level.levels = shuffle(level.levels); //shuffles order of maps
	        level.levels.splice(0, level.cgonLevels.length)
            }
